...
- **`/api/files`**：`server.py` が `files/` を走査して返すJSONです。
  `/api/files.zip` は全ファイルをまとめたZIPを生成します（合計512MBまで）。
- **まとめてダウンロード（ZIP）**：`server.py` 配信中はサーバーがZIPを作ります。
  静的配信のときは `assets/js/zip.js` がブラウザ内でZIPを組み立てるので、
  どちらの配信方法でも使えます（外部ライブラリなし・無圧縮のstore方式）。
  ZIP名は `taiken-materials-YYYYMMDD.zip`。中のフォルダ名は日本語のままです
  （ZIPのファイル名自体を日本語にすると、ブラウザによって拡張子が失われるため半角にしています）。
